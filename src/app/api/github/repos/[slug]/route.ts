...
// Only repos listed in BEADS_GITHUB_REPOS are accepted, so this endpoint can
// never be used to clone arbitrary URLs.
export async function GET(
  ...
  const repo = parseGithubRepos(process.env.BEADS_GITHUB_REPOS).find((r) => r.slug === slug);
  if (!repo) {
